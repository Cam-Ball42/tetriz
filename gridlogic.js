...
export default class Grid {
        ...
        get_downward_projection() {
                if (this.active_shape !== null) {
                        let hit_bot = false;
                        let step = 1;
                        const projected_shape = structuredClone(this.active_shape);
                        while (!hit_bot) {
                                if (this.check_move(projected_shape, 0, step) === false) {
                                        step += 1;
                                } else {
                                        hit_bot = true;
                                }
                        }
                        const projected_points = projected_shape.points;
                        for (const point of projected_points) {
                                point.x = point.x + this.active_shape.pos.x;
                                point.y = point.y + this.active_shape.pos.y + step - 1;
                        }
        ...
        remove_full_lines(full_lines) {
                ...
                drawTools.particlize_line(full_lines[0]);
